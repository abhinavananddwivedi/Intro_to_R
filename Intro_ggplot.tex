...
``types'', say numeric and categorical and character and logical all
together.
...
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{filter}\NormalTok{(country }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"China"}\NormalTok{, }\StringTok{"India"}\NormalTok{, }\StringTok{"France"}\NormalTok{, }\StringTok{"Germany"}\NormalTok{))}
...
example, the default statistic for geom\_bar is stat\_bin:
...
\includegraphics{Intro_ggplot_files/figure-latex/plot_eur_box_flip-1.pdf}
...
\includegraphics{Intro_ggplot_files/figure-latex/CIFG_theme-1.pdf}
...
\includegraphics{Intro_ggplot_files/figure-latex/CIFG_theme-2.pdf}
...
\includegraphics{Intro_ggplot_files/figure-latex/CIFG_theme-3.pdf}

\section{The Main FAQs}\label{the-main-faqs}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  plotting multiple variables in one frame
\end{enumerate}

Map Aesthetic To Different Columns The most frequently asked question
goes something like this: I have two variables in my data.frame, and I'd
like to plot them as separate points, with different color depending on
which variable it is. How do I do that?

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{temp <-}\StringTok{ }\KeywordTok{cbind}\NormalTok{(}\DecValTok{1991}\OperatorTok{:}\DecValTok{2010}\NormalTok{, }\KeywordTok{rnorm}\NormalTok{(}\DecValTok{20}\NormalTok{, }\DecValTok{0}\NormalTok{, }\DecValTok{1}\NormalTok{), }\KeywordTok{rnorm}\NormalTok{(}\DecValTok{20}\NormalTok{, }\OperatorTok{-}\DecValTok{1}\NormalTok{, }\DecValTok{5}\NormalTok{)) }\OperatorTok{%>%}
\StringTok{  }\NormalTok{tibble}\OperatorTok{::}\KeywordTok{as_tibble}\NormalTok{()}
\KeywordTok{names}\NormalTok{(temp) <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"year"}\NormalTok{, }\StringTok{"N01"}\NormalTok{, }\StringTok{"N25"}\NormalTok{)}

\NormalTok{(}\KeywordTok{ggplot}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ temp,}
...
\StringTok{    }\KeywordTok{geom_line}\NormalTok{(}\DataTypeTok{mapping =} \KeywordTok{aes}\NormalTok{(}\DataTypeTok{y =}\NormalTok{ N25), }\DataTypeTok{color =} \StringTok{"blue"}\NormalTok{)}
\NormalTok{  )}
\end{Highlighting}
\end{Shaded}
